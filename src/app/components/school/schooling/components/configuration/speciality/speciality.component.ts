...
export class SpecialityComponent implements OnInit {
  ...
  constructor(public specialityService: SpecialityService, config: NgbModalConfig,
  ...
  getSpecialityList(): any {
    this.specialityService.getCollection().subscribe({
      next: (res: any) => {
        this.specialitys = res['hydra:member'];

        console.log(this.specialitys);

        // cache our list
        this.temp = [...res['hydra:member']];

        this.rows = this.specialitys;
  ...
  importSpeciality(): void {
    this.router.navigateByUrl('/school/schooling/import/speciality');
  }

  addSpeciality() {
    const modalRef = this.modalService.open(SpecialityAddEditComponent, {
      // centered: true,
    });
