...
export class DepartmentComponent implements OnInit {
  ...
  constructor(public departmentService: DepartmentService, config: NgbModalConfig,
  ...
  getDepartmentList(): any {
    this.departmentService.getCollection().subscribe({
      next: (res: any) => {
  ...
  importDepartment(): void {
    this.router.navigateByUrl('/school/schooling/import/department');
  }

  addDepartment() {
    const modalRef = this.modalService.open(DepartmentAddEditComponent, {
      // centered: true,
    });
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.departmentService.delete(id).subscribe({
          next: (res) => {
            this.toastr.success('Department successful deleted', 'Success');
            this.getDepartmentList();
          },
          error: (res) => {
            this.loadingIndicator = false;
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.departmentService.deleteMultiple(ids).subscribe({
          next: (res) => {
            console.log(res);
            this.toastr.success(`Department's deleted !`, 'Success');
            this.getDepartmentList();
            this.remove();
          },
          error: (err: any) => {
