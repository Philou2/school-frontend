...
export class ClassProgramComponent implements OnInit {
  ...
  public ClassProgrames!: ClassProgram[];
  temp = [];
  selected = [];
  ...
  getClassProgramlist(): any
  {
    this.classProgramService.getCollection().subscribe({
      next: (res: any) => {
        console.log(res);
        this.ClassProgrames = res['hydra:member'];

        console.log(this.ClassProgrames);

        // cache our list
        // this.temp = [...res['hydra:member']];
        this.temp = [...res['hydra:member']];

        this.rows = this.ClassProgrames;
  ...
  // addClassProgram() {
  //   const modalRef = this.modalService.open(ClassProgramAddEditComponent, {
  //     // centered: true,
  ...
  addClassProgram() {
    this.router.navigate(['/school/study/add-edit-class-program']);
  }

  editClassProgram(data: any) {
    console.log(data);
  ...
  deleteClassProgram(id: number): any{
    Swal.fire({
      title: 'Are you sure?',
      ...
      if (result.isConfirmed) {
        ...
        this.classProgramService.delete(id).subscribe({
          next: (res:any) => {
            if (res.deleted) {
              this.toastr.success('Class Program successful deleted', 'Success');
              this.getClassProgramlist();
            } else {
              this.toastr.warning('This class program has marks','Warning');
            }
          },
          error: (res) => {
            this.loadingIndicator = false;
  ...
  deleteSelectedClassProgramRows() {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.classProgramService.deleteMultiple(ids).subscribe({
          next: (res) => {
            console.log(res);
            this.toastr.success('Class Program deleted !', 'Success');
            this.getClassProgramlist();
            this.remove();
          },
          error: (err: any) => {
