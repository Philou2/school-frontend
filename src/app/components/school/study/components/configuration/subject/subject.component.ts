...
export class SubjectComponent implements OnInit {
  ...
  constructor(public subjectService: SubjectService, config: NgbModalConfig,
  ...
  getSubjectlist(): any
  {
    this.subjectService.getCollection().subscribe({
      next: (res: any) => {
        console.log(res);
        this.subjects = res['hydra:member'];
  ...
  importSubject(): void {
    this.router.navigateByUrl('/school/study/import/subject');
  }

  addSubject() {
    const modalRef = this.modalService.open(SubjectAddEditComponent);
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.subjectService.delete(id).subscribe({
          next: (res) => {
            this.toastr.success('Subject successful deleted', 'Success');
            this.getSubjectlist();
          },
          error: (res) => {
            this.loadingIndicator = false;
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.subjectService.deleteMultiple(ids).subscribe({
          next: (res) => {
            console.log(res);
            this.toastr.success('Subject deleted !', 'Success');
            this.getSubjectlist();
            this.remove();
          },
          error: (err: any) => {
